...
function lockExistingDaysDates() {
  if (!days || !days.length) return;
  for (let i = 0; i < days.length; i++) {
    if (!days[i].dateOverride) {
      days[i].dateOverride = formatDateLocal(getDd(i));
    }
  }
}

function hasDayProgress(dayObj) {
  if (!dayObj || !dayObj.blocks) return false;
  return dayObj.blocks.some(b => {
    const p = prog[b.id];
    if (!p) return false;
    if (p.timeSpent && p.timeSpent > 0) return true;
    if (p.notes && p.notes.trim().length > 0) return true;
    if (p.customTasks && p.customTasks.length > 0) return true;
    if (p.subtopics && Object.values(p.subtopics).some(val => val === true)) return true;
    return false;
  });
}

function isHistoricalDay(dayObj) {
...
function sortDaysByDate() {
  if (!days || !days.length) return;
  days.sort((a, b) => {
    const dA = a.dateOverride ? parseDateLocal(a.dateOverride) : new Date(0);
    const dB = b.dateOverride ? parseDateLocal(b.dateOverride) : new Date(0);
    return dA.getTime() - dB.getTime();
  });

  days.forEach((d, idx) => {
    if (!d.title || d.title.startsWith('Day ') || d.title === 'Study Day') {
      d.title = `Day ${idx + 1}`;
    }
  });
}

function checkPlanExpirationAndSetCurDay() {
  ...
  let lastDate = getDd(0);
  for (let i = 0; i < days.length; i++) {
    const dDate = getDd(i);
    if (dDate > lastDate) lastDate = dDate;
  }
  lastDate.setHours(0, 0, 0, 0);

  if (today > lastDate) {
    // Today is past the last date of the plan period (e.g. Sept 1 after Aug 31)
    // Do NOT jump back to Day 1 (July 1st). Focus on last day and prompt user to adjust/extend.
    curDay = days.length - 1;
    setTimeout(() => {
      const endDateEl = document.getElementById('planExpiredEndDate');
      if (endDateEl) {
        endDateEl.textContent = fd(lastDate);
      }
      openModal('planExpiredOverlay');
    }, 600);
  } else {
    curDay = 0;
  }
...
function acceptCreateWeekPlan() {
  closeModal('planExpiredOverlay');
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const end = new Date(today);
  end.setDate(end.getDate() + 6);
  createWeekPlanForDates(today, end);
}

function openAdjustDatesFromExpired() {
  closeModal('planExpiredOverlay');
  openDateRangePicker();
}

function generateRandomBlocksForDay() {
  if (!subj || subj.length === 0) {
    return [
      { id: gid(), subjectId: 's1', targetHrs: 4, topic: 'General Topic 1', subtopics: ['Subtopic 1', 'Subtopic 2'] },
      { id: gid(), subjectId: 's2', targetHrs: 4, topic: 'General Topic 2', subtopics: ['Subtopic 1', 'Subtopic 2'] }
    ];
  }

  const availableSubj = [...subj];
  const numBlocks = Math.min(availableSubj.length, Math.floor(Math.random() * 2) + 2);
  const selectedSubj = [];

  for (let i = 0; i < numBlocks; i++) {
    const rIdx = Math.floor(Math.random() * availableSubj.length);
    selectedSubj.push(availableSubj.splice(rIdx, 1)[0]);
  }

  const hoursDistribution = numBlocks === 2 ? [4, 4] : [3, 3, 2];

  return selectedSubj.map((s, idx) => {
    let chosenTopic = 'Topic 1';
    let chosenSubtopics = ['Subtopic 1', 'Subtopic 2'];

    try {
      if (typeof getExistingTopicsForSubject === 'function') {
        const topicObjList = getExistingTopicsForSubject(s.id);
        if (Array.isArray(topicObjList) && topicObjList.length > 0) {
          const rObj = topicObjList[Math.floor(Math.random() * topicObjList.length)];
          if (rObj && rObj.topic) {
            chosenTopic = rObj.topic;
            const subList = Array.isArray(rObj.subtopics) ? rObj.subtopics : [];
            if (subList.length > 0) {
              const count = Math.min(subList.length, 3);
              const startSub = Math.floor(Math.random() * Math.max(1, subList.length - count + 1));
              chosenSubtopics = subList.slice(startSub, startSub + count);
            }
          }
        }
      }
    } catch (e) {
      console.error("Error generating random block topic:", e);
    }

    return {
      id: gid(),
      subjectId: s.id,
      targetHrs: hoursDistribution[idx] || 3,
      topic: chosenTopic,
...
function createWeekPlanForDates(startDate, endDate) {
  ...
  const historicalDays = days.filter(d => isHistoricalDay(d));
  const newDaysList = [...historicalDays];
  ...
  for (let i = 0; i < numDays; i++) {
    ...
    const existingHist = historicalDays.find(d => {
      const dd = d.dateOverride ? parseDateLocal(d.dateOverride) : null;
      return dd && formatDateLocal(dd) === dateStr;
    });

    if (!existingHist) {
      const newDayNum = newDaysList.length + 1;
      newDaysList.push({
        id: 'day_' + Date.now() + '_' + i + '_' + Math.random().toString(36).slice(2, 5),
        title: `Day ${newDayNum}`,
        dateOverride: dateStr,
        targetHrs: 8,
        blocks: generateRandomBlocksForDay()
      });
    }
  }
  ...
  days = newDaysList;
  sortDaysByDate();
  conf.targetDate = formatDateLocal(end);
  sd();
  ...
  if (typeof updateDaysRemaining === 'function') updateDaysRemaining();
}
...
function openDateRangePicker() {
  ...
  if (rangeStartDate < today) {
    calCurrentMonth = new Date(today);
  } else {
...
function changeModalStartDate(val) {
  ...
  rangeStartDate.setHours(0, 0, 0, 0);
  if (!rangeEndDate || rangeEndDate < rangeStartDate) {
...
function updateSelectRangeButton() {
  ...
  const s = rangeStartDate || rangeEndDate;
  const e = rangeEndDate || rangeStartDate;
  ...
  btn.disabled = false;
  btn.textContent = `Select ${totalDays} ${totalDays === 1 ? 'day' : 'days'}`;
}
...
function renderCalendar() {
  ...
  for (let day = 1; day <= numDays; day++) {
    ...
    let inRange = false;
    if (rangeStartDate && rangeEndDate) {
      const minT = Math.min(rangeStartDate.getTime(), rangeEndDate.getTime());
      const maxT = Math.max(rangeStartDate.getTime(), rangeEndDate.getTime());
      inRange = dayDate.getTime() > minT && dayDate.getTime() < maxT;
    }
...
function applySelectedDateRange() {
  try {
    const rStart = rangeStartDate || rangeEndDate;
    const rEnd = rangeEndDate || rangeStartDate;
    if (!rStart || !rEnd) return;

    lockExistingDaysDates();

    const minTime = Math.min(rStart.getTime(), rEnd.getTime());
    const maxTime = Math.max(rStart.getTime(), rEnd.getTime());
    ...
    conf.startDate = startStr;
    conf.targetDate = endStr;
    sc();

    // Preserve ONLY past dates (date < today) OR days with recorded user progress/notes/time
    const historicalDays = days.filter(d => isHistoricalDay(d));
    const newDaysList = [...historicalDays];
    ...
    if (activeIdx !== -1) {
      ...
      const firstOfRange = days.findIndex((d, idx) => formatDateLocal(getDd(idx)) === startStr);
      curDay = firstOfRange !== -1 ? firstOfRange : days.length - 1;
    }
  } catch (err) {
    console.error("Error applying selected date range:", err);
  } finally {
    updateDaysRemaining();
...
function generatePlanTillTargetDate() {
  ...
  const targetDt = parseDateLocal(conf.targetDate);
  targetDt.setHours(0, 0, 0, 0);

  let latestDt = getDd(0);
  for (let i = 0; i < days.length; i++) {
    const dDate = getDd(i);
    if (dDate > latestDt) latestDt = dDate;
  }
  latestDt.setHours(0, 0, 0, 0);

  if (targetDt <= latestDt) {
    alert(`Plan already covers up to ${fd(latestDt)}. Earlier data is preserved.`);
    return;
  }

  const daysToAdd = Math.ceil((targetDt.getTime() - latestDt.getTime()) / (1000 * 60 * 60 * 24));
  for (let i = 1; i <= daysToAdd; i++) {
    const curDate = new Date(latestDt.getTime() + i * 86400000);
    const dateStr = formatDateLocal(curDate);
  ...
  renderManage();
  updateDaysRemaining();
  alert(`Plan extended to ${fd(targetDt)} (${days.length} total days preserved).`);
}
...
function renderStats() {
  ...
  if (statsRow) {
    ...
    if (activeTab !== 'daily' || conf.trackerMode === 'easy' || conf.theme === 'cyberpunk' || conf.theme === 'stopwatch') {
  ...
  if (conf.theme === 'neonquest') {
    const progPct = tot ? Math.round(done / tot * 100) : 0;
    const totalMins = Math.floor(totalSec / 60);
    document.getElementById('statsRow').innerHTML = `
      <!-- 1. PROGRESS -->
      <div class="stat-card clickable group" onclick="showStatsDetail('progress')" style="cursor:pointer;position:relative;padding:14px 16px;display:flex;flex-direction:column;justify-content:space-between">
        <div style="text-align:center;font-size:11px;font-weight:700;letter-spacing:0.12em;color:#cbd5e1;text-transform:uppercase">PROGRESS</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:12px;margin:8px 0">
          <div style="font-size:32px;filter:drop-shadow(0 0 10px rgba(251,191,36,0.7))">🏆</div>
          <div style="display:flex;flex-direction:column">
            <span style="font-size:22px;font-weight:700;color:#f8fafc;line-height:1">${progPct}%</span>
            <span style="font-size:11px;color:#94a3b8;margin-top:4px">(${done}/${tot} Levels Done)</span>
          </div>
        </div>
        <div style="width:100%;height:6px;background:#030712;border-radius:9999px;overflow:hidden;border:1px solid rgba(6,182,212,0.4)">
          <div style="height:100%;width:${Math.max(5, progPct)}%;background:linear-gradient(90deg, #06b6d4, #38bdf8);box-shadow:0 0 8px #38bdf8;border-radius:9999px"></div>
        </div>
      </div>

      <!-- 2. AVIRAL (STREAK) -->
      <div class="stat-card clickable group" onclick="showStatsDetail('streak')" style="cursor:pointer;position:relative;padding:14px 16px;display:flex;flex-direction:column;justify-content:space-between">
        <div style="text-align:center;font-size:11px;font-weight:700;letter-spacing:0.12em;color:#cbd5e1;text-transform:uppercase">AVIRAL (STREAK)</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:12px;margin:8px 0">
          <div style="font-size:32px;filter:drop-shadow(0 0 12px rgba(249,115,22,0.85))">🔥</div>
          <div style="display:flex;flex-direction:column">
            <span style="font-size:22px;font-weight:700;color:#f8fafc;line-height:1">${streak} DAYS</span>
          </div>
        </div>
        <div style="display:flex;justify-content:center;margin-bottom:-4px">
          <span style="padding:2px 10px;font-size:10px;font-weight:800;border-radius:9999px;background:#121c32;color:#fb923c;border:1px solid rgba(249,115,22,0.7);box-shadow:0 0 8px rgba(249,115,22,0.4)">COMBO</span>
        </div>
      </div>

      <!-- 3. TIME LOGGED -->
      <div class="stat-card clickable group" onclick="showStatsDetail('time')" style="cursor:pointer;position:relative;padding:14px 16px;display:flex;flex-direction:column;justify-content:space-between">
        <div style="text-align:center;font-size:11px;font-weight:700;letter-spacing:0.12em;color:#cbd5e1;text-transform:uppercase">TIME LOGGED</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:12px;margin:8px 0">
          <div style="font-size:32px;filter:drop-shadow(0 0 10px rgba(6,182,212,0.8))">⏱️</div>
          <div style="display:flex;flex-direction:column">
            <span style="font-size:20px;font-weight:700;color:#f8fafc;line-height:1">${totalMins} MINS</span>
            <span style="font-size:10px;color:#94a3b8;margin-top:4px;font-weight:600">TOTAL</span>
          </div>
        </div>
        <div style="display:flex;justify-content:center;margin-bottom:-4px">
          <span style="padding:2px 12px;font-size:10px;font-weight:800;border-radius:9999px;background:#121c32;color:#38bdf8;border:1px solid rgba(6,182,212,0.7);box-shadow:0 0 8px rgba(6,182,212,0.4)">XP</span>
        </div>
      </div>

      <!-- 4. TODAY'S GOAL -->
      <div class="stat-card clickable group" onclick="showStatsDetail('today')" style="cursor:pointer;position:relative;padding:14px 16px;display:flex;flex-direction:column;justify-content:space-between">
        <div style="text-align:center;font-size:11px;font-weight:700;letter-spacing:0.12em;color:#cbd5e1;text-transform:uppercase">TODAY'S GOAL</div>
        <div style="display:flex;align-items:center;justify-content:center;gap:12px;margin:8px 0">
          <div style="position:relative;display:flex;align-items:center;justify-content:center;font-size:24px">
            <span>📅</span>
            <span style="position:absolute;bottom:-4px;right:-6px;font-size:14px">🎯</span>
          </div>
          <div style="display:flex;flex-direction:column">
            <span style="font-size:20px;font-weight:700;color:#f8fafc;line-height:1">${todayLog}h / ${todayTgt}h</span>
            <span style="font-size:10px;color:#94a3b8;margin-top:4px;font-weight:600">GOAL</span>
          </div>
        </div>
        <div style="width:100%;height:6px;background:#030712;border-radius:9999px;overflow:hidden;border:1px solid rgba(139,92,246,0.4)">
          <div style="height:100%;width:${Math.min(100, Math.round((todayLog / todayTgt) * 100))}%;background:linear-gradient(90deg, #8b5cf6, #ec4899);box-shadow:0 0 8px #c084fc;border-radius:9999px"></div>
        </div>
      </div>
    `;
    renderHoursBar();
...
function renderHoursBar() {
  ...
  const activeTab = conf.activeTab || 'daily';
  if (hoursBar && (activeTab !== 'daily' || conf.trackerMode === 'easy' || conf.theme === 'cyberpunk' || conf.theme === 'stopwatch')) {
  ...
  if (conf.theme === 'neonquest') {
    const segs = d.blocks.map((b, idx) => {
      const s = sj(b.subjectId);
      const pct = Math.min((b.targetHrs / target) * 100, 100);
      const colors = ['#00b4d8', '#f97316', '#fbbf24', '#8b5cf6', '#22c55e'];
      const barCol = colors[idx % colors.length] || s.color;
      return `<div style="width:${pct}%;height:100%;background:${barCol};display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;color:#000;box-shadow:inset 0 0 6px rgba(0,0,0,0.3)" title="${s.name}: ${b.targetHrs}h">${b.targetHrs}h</div>`;
    }).join('');

    const legend = d.blocks.map((b, idx) => {
      const s = sj(b.subjectId);
      const colors = ['#00b4d8', '#f97316', '#fbbf24', '#8b5cf6', '#22c55e'];
      const dotCol = colors[idx % colors.length] || s.color;
      return `<div style="display:flex;align-items:center;gap:6px;font-size:12px;font-weight:700;color:#e2e8f0"><div style="width:8px;height:8px;border-radius:50%;background:${dotCol};box-shadow:0 0 6px ${dotCol}"></div>${esc(s.name)}</div>`;
    }).join('');

    document.getElementById('hoursBar').innerHTML = `
      <div style="background:#091326;border:1.5px solid #1e2d4a;border-radius:14px;padding:14px 18px;box-shadow:0 0 16px rgba(0,0,0,0.4)">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
          <div style="font-family:'Orbitron',sans-serif;font-size:13px;font-weight:800;letter-spacing:0.08em;color:#f8fafc">DAY ${curDay + 1} XP PLAN</div>
          <div style="font-size:12px;color:#94a3b8;font-weight:600">Planned: <strong style="color:#f8fafc">${planned}h</strong> • Target: <strong style="color:#f8fafc">${target}h</strong></div>
        </div>
        <div style="width:100%;height:14px;background:#030712;border-radius:9999px;overflow:hidden;border:1px solid #1e2d4a;display:flex">
          ${segs || '<div style="padding:2px 8px;font-size:10px;color:#64748b">No subjects</div>'}
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:12px;flex-wrap:wrap;gap:8px">
          <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">${legend}</div>
          <div style="font-size:12px;color:#38bdf8;font-weight:700;text-shadow:0 0 8px rgba(56,189,248,0.6)">Keep going (${logStr} / ${target}h)</div>
        </div>
      </div>`;
    return;
  }
...
function renderEasyModeTick() {
  checkDailyRollover();
  if (conf.trackerMode !== 'easy' && conf.theme !== 'stopwatch') return;
  ...
  const totalSec = days.reduce((s, d) => s + d.blocks.reduce((ss, bk) => ss + (gp(bk.id).timeSpent || 0) + (timers[bk.id]?.running ? Math.floor((Date.now() - timers[bk.id].start) / 1000) : 0), 0), 0);
  const th = Math.floor(totalSec / 3600), tm = Math.floor((totalSec % 3600) / 60);
  const totalMinsText = th ? `${th}h ${tm}m` : `${tm} mins`;
  
  const todayI = days.findIndex((_, i) => isToday(getDd(i)));
  const todayActiveSec = (curDayObj?.blocks || []).reduce((sum, bk) => sum + (gp(bk.id).timeSpent || 0) + (timers[bk.id]?.running ? Math.floor((Date.now() - timers[bk.id].start) / 1000) : 0), 0);
  const todayLog = (todayActiveSec / 3600).toFixed(1);
  const todayTgt = curDayObj?.targetHrs || (todayI >= 0 ? (days[todayI].targetHrs || 8) : 8);

  const progValEl = document.getElementById('easyStatProgressVal');
  if (progValEl) progValEl.textContent = `${tot ? Math.round(done / tot * 100) : 0}%`;
  const progSubEl = document.getElementById('easyStatProgressSub');
  if (progSubEl) progSubEl.textContent = `${done}/${tot} days done`;

  const streakValEl = document.getElementById('easyStatStreakVal');
  if (streakValEl) streakValEl.textContent = `${streak}`;

  const timeValEl = document.getElementById('easyStatTimeVal');
  if (timeValEl) timeValEl.textContent = totalMinsText;

  const todayValEl = document.getElementById('easyStatTodayVal');
  if (todayValEl) todayValEl.textContent = `${todayLog}h`;
  const todaySubEl = document.getElementById('easyStatTodaySub');
  if (todaySubEl) todaySubEl.textContent = `of ${todayTgt}h target`;

  const hpHeaderMeta = document.getElementById('easyHoursPlanHeaderMeta');
  if (hpHeaderMeta) {
    hpHeaderMeta.innerHTML = `In Progress: <strong style="color:#38bdf8">${todayLog}h</strong> / ${todayTgt}h · Planned: ${dPlannedHrs(curDay) || todayTgt}h`;
  }

  // Update running timer card and button
  (curDayObj?.blocks || []).forEach(b => {
    const isRunning = !!timers[b.id]?.running;
    const btnEl = document.getElementById('easyBtn-' + b.id);
    if (btnEl) {
      if (isRunning) {
        btnEl.innerHTML = `<span>Pause</span><span style="font-size:10px">⏸</span>`;
        btnEl.style.background = '#fbbf24';
      } else {
        btnEl.innerHTML = `<span>Start</span><span style="font-size:10px">▶</span>`;
  ...
  if (runningBid) {
    ...
    const clockStr = `${String(ah).padStart(2, '0')}:${String(am).padStart(2, '0')}:${String(as).padStart(2, '0')}`;

    const clockEl = document.getElementById('easyClock-' + runningBid);
    if (clockEl) clockEl.textContent = clockStr;

    const b = (curDayObj?.blocks || []).find(x => x.id === runningBid);
    if (b) {
      const targetSec = (b.targetHrs || 3) * 3600;
      const pctVal = Math.min(100, Math.round((activeSec / targetSec) * 100));
      const hoursDec = (activeSec / 3600).toFixed(1);

      const pctEl = document.getElementById('easyPct-' + runningBid);
      if (pctEl) pctEl.textContent = `${pctVal}%`;

      const rightPctEl = document.getElementById('easyRightPct-' + runningBid);
      if (rightPctEl) rightPctEl.textContent = `${pctVal}%`;

      const hrsEl = document.getElementById('easyHrs-' + runningBid);
      if (hrsEl) hrsEl.textContent = `${hoursDec}/${b.targetHrs} hours`;

      const barEl = document.getElementById('easyBar-' + runningBid);
      if (barEl) barEl.style.width = `${pctVal}%`;

      const hpFill = document.getElementById('easyHpFill-' + runningBid);
      if (hpFill) hpFill.style.width = `${Math.max(pctVal > 0 ? 5 : 0, pctVal)}%`;

      const hpBadge = document.getElementById('easyHpBadge-' + runningBid);
      if (hpBadge) hpBadge.textContent = `${hoursDec}h / ${b.targetHrs}h (${pctVal}%)`;

      const hpRight = document.getElementById('easyHpRowRight-' + runningBid);
      if (hpRight) hpRight.textContent = `${hoursDec}h / ${b.targetHrs}h`;
    }
  }
}
...
function renderEasyModeView() {
  ...
  const totalDays = days.length || 61;
  const dayDate = getDd(curDay);
  const dateStr = dayDate.toLocaleDateString('en-US', { weekday: 'short', day: 'numeric', month: 'short' });

  // 1. Calculate Stats accurately matching standard tracker logic
  const done = days.filter((_, i) => dPct(i) === 100).length;
  const tot = days.length || 61;
  ...
  const todayI = days.findIndex((_, i) => isToday(getDd(i)));
  const todayActiveSec = (curDayObj?.blocks || []).reduce((sum, bk) => sum + (gp(bk.id).timeSpent || 0) + (timers[bk.id]?.running ? Math.floor((Date.now() - timers[bk.id].start) / 1000) : 0), 0);
  const todayHoursFormatted = (todayActiveSec / 3600).toFixed(1) + 'h';
  const targetDailyHours = curDayObj?.targetHrs || (todayI >= 0 ? (days[todayI].targetHrs || 8) : 8);

  // 2. Day Selector Carousel (5 Days Centered around curDay)
  const visibleCount = 5;
  const half = Math.floor(visibleCount / 2);
  let startDayNum = Math.max(1, dayNum - half);
  let endDayNum = startDayNum + visibleCount - 1;
  if (endDayNum > totalDays) {
    endDayNum = totalDays;
    startDayNum = Math.max(1, endDayNum - visibleCount + 1);
  }

  let daySelectorHtml = '';
  for (let d = startDayNum; d <= endDayNum; d++) {
    const isSelected = d === dayNum;
    const dDate = getDd(d - 1);
    const dDateStr = dDate.toLocaleDateString('en-US', { weekday: 'short', day: 'numeric', month: 'short' });

    if (isSelected) {
      daySelectorHtml += `
        <button onclick="jumpTo(${d - 1})" style="background:#38bdf8;border:none;border-radius:12px;padding:8px 12px;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;color:#ffffff;box-shadow:0 4px 14px rgba(56,189,248,0.4);outline:none">
          <span style="font-size:13px;font-weight:800;color:#ffffff;line-height:1.2">Day ${d}</span>
          <span style="font-size:10px;font-weight:600;color:#f0f9ff;margin-top:2px;white-space:nowrap">${dDateStr}</span>
        </button>`;
    } else {
      daySelectorHtml += `
        <button onclick="jumpTo(${d - 1})" style="background:transparent;border:none;border-radius:12px;padding:8px 12px;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;color:#94a3b8;outline:none" onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='transparent'">
          <span style="font-size:13px;font-weight:700;color:#cbd5e1;line-height:1.2">${d}</span>
          <span style="font-size:10px;color:#64748b;margin-top:2px;white-space:nowrap">${dDateStr}</span>
        </button>`;
    }
  }

  // 3. Hours Plan (Stacked Subject Bars with live moving color progress)
  let totalPlannedHours = 0;
  const hoursPlanBarsHtml = (curDayObj?.blocks || []).map(b => {
    const s = sj(b.subjectId);
    const p = gp(b.id);
    const plannedHrs = b.targetHrs || s.defaultHrs || 3;
    totalPlannedHours += plannedHrs;
    const bSec = (p.timeSpent || 0) + (timers[b.id]?.running ? Math.floor((Date.now() - timers[b.id].start) / 1000) : 0);
    const bLoggedH = (bSec / 3600).toFixed(1);
    const pctLogged = Math.min(100, Math.round((bSec / (plannedHrs * 3600)) * 100));
    const fillPercent = Math.min(100, Math.max(pctLogged > 0 ? 5 : 0, pctLogged));

    return `
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
        <div style="position:relative;flex:1;height:38px;background:#22232a;border-radius:9999px;overflow:hidden;display:flex;align-items:center">
          <!-- Moving Color Progress Fill -->
          <div id="easyHpFill-${b.id}" style="position:absolute;left:0;top:0;bottom:0;border-radius:9999px;background-color:${s.color};width:${fillPercent}%;box-shadow:0 0 10px ${s.color}60;transition:width 0.4s ease-out"></div>

          <!-- Subject Content Overlay -->
          <div style="position:relative;z-index:1;width:100%;height:100%;display:flex;align-items:center;justify-content:space-between;padding:0 14px;pointer-events:none">
            <div style="display:flex;align-items:center;gap:8px;color:#ffffff;font-size:12px;font-weight:800;text-shadow:0 1px 3px rgba(0,0,0,0.8);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
              <span style="font-size:14px">${s.icon || '🎓'}</span>
              <span style="overflow:hidden;text-overflow:ellipsis">${esc(s.name)}</span>
            </div>
            <span id="easyHpBadge-${b.id}" style="background:rgba(0,0,0,0.45);backdrop-filter:blur(4px);color:#ffffff;font-size:11px;font-weight:800;padding:2px 8px;border-radius:9999px;margin-left:6px;border:1px solid rgba(255,255,255,0.15)">
              ${bLoggedH}h / ${plannedHrs}h (${pctLogged}%)
            </span>
          </div>
        </div>
        <span id="easyHpRowRight-${b.id}" style="font-size:12px;font-weight:700;color:#94a3b8;min-width:65px;text-align:right;flex-shrink:0">${bLoggedH}h / ${plannedHrs}h</span>
      </div>`;
  }).join('');

  // 4. Subject Timer Cards List (matching downloaded SubjectTimerCard.tsx)
  const subjectTimerCardsHtml = (curDayObj?.blocks || []).map(b => {
    const s = sj(b.subjectId);
    const p = gp(b.id);
    let sec = p.timeSpent || 0;
    const isRunning = !!timers[b.id]?.running;
    if (isRunning) {
      sec += Math.floor((Date.now() - timers[b.id].start) / 1000);
    }

    const th = Math.floor(sec / 3600), tm = Math.floor((sec % 3600) / 60), ts = sec % 60;
    const clockStr = `${String(th).padStart(2, '0')}:${String(tm).padStart(2, '0')}:${String(ts).padStart(2, '0')}`;
    const targetSec = (b.targetHrs || 3) * 3600;
    const bPctVal = Math.min(100, Math.round((sec / targetSec) * 100));
    const loggedHoursDec = (sec / 3600).toFixed(1);

    return `
      <div style="position:relative;background:#17181c;border:1px solid #23242a;border-radius:16px;padding:14px 18px;margin-bottom:12px;box-shadow:0 4px 20px rgba(0,0,0,0.25);overflow:hidden;transition:all 0.2s">
        <!-- Left Vertical Subject Color Accent Bar -->
        <div style="position:absolute;left:0;top:0;bottom:0;width:4px;border-top-left-radius:16px;border-bottom-left-radius:16px;background-color:${s.color}"></div>

        <div style="display:flex;align-items:center;justify-content:space-between;gap:14px;padding-left:6px;flex-wrap:wrap">
          <!-- Left: Monospace Clock & White Start/Pause Pill Button -->
          <div style="display:flex;align-items:center;gap:12px;flex-shrink:0">
            <div style="font-family:'Space Mono',monospace;font-size:16px;font-weight:700;letter-spacing:0.06em;color:#ffffff;min-width:78px" id="easyClock-${b.id}">
              ${clockStr}
            </div>

            <button id="easyBtn-${b.id}" onclick="toggleTimer('${b.id}','${curDayObj.id}')"
                    style="display:inline-flex;align-items:center;gap:6px;padding:6px 14px;border-radius:9999px;font-weight:800;font-size:12px;cursor:pointer;border:none;box-shadow:0 2px 8px rgba(0,0,0,0.3);background:${isRunning ? '#fbbf24' : '#ffffff'};color:#09090b;transition:all 0.15s ease">
              <span>${isRunning ? 'Pause' : 'Start'}</span>
              <span style="font-size:10px">${isRunning ? '⏸' : '▶'}</span>
            </button>

            ${sec > 0 && !isRunning ? `
              <button onclick="resetTimerBlock('${b.id}','${curDayObj.id}')" title="Reset Timer" style="background:transparent;border:none;color:#64748b;cursor:pointer;font-size:14px;padding:4px">
                🔄
              </button>` : ''}
          </div>

          <!-- Middle: Subject Info, Progress & Topic -->
          <div style="flex:1;min-width:180px">
            <div style="display:flex;align-items:center;gap:6px;font-size:12px;color:#94a3b8;margin-bottom:6px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
              <span style="font-weight:700;color:#ffffff;overflow:hidden;text-overflow:ellipsis">${esc(s.name)}</span>
              <span>-</span>
              <span style="color:#e2e8f0;font-weight:600" id="easyPct-${b.id}">${bPctVal}%</span>
              <span>·</span>
              <span style="color:#94a3b8" id="easyHrs-${b.id}">${loggedHoursDec}/${b.targetHrs} hours</span>
              <span>·</span>
              <span style="color:#94a3b8;cursor:pointer;overflow:hidden;text-overflow:ellipsis" onclick="editTopicModal('${curDayObj.id}','${b.id}')" title="Click to edit topic">
                ${esc(b.topic || 'Topic 2')}
              </span>
            </div>

            <!-- Thin Progress Bar -->
            <div style="width:100%;height:4px;background:#22232a;border-radius:9999px;overflow:hidden">
              <div id="easyBar-${b.id}" style="height:100%;border-radius:9999px;background-color:${s.color};width:${bPctVal}%;transition:width 0.4s ease-out"></div>
            </div>
          </div>

          <!-- Right Target Info -->
          <div style="text-align:right;flex-shrink:0">
            <div style="font-size:11px;color:#64748b;margin-bottom:2px">${b.targetHrs} hours target</div>
            <div style="font-size:12px;font-weight:800;color:#e2e8f0" id="easyRightPct-${b.id}">${bPctVal}%</div>
          </div>
        </div>
      </div>`;
  }).join('');

  // 5. Render Full Layout Container
  container.innerHTML = `
    <div style="max-width:860px;margin:0 auto;padding:12px 6px">
      ...
      <div style="display:flex;justify-content:flex-end;align-items:center;margin-bottom:14px">
        <div style="display:flex;align-items:center;gap:6px">
          <select id="quickThemeSelectEasy" onchange="setAppTheme(this.value)"
            ...
            <option value="stopwatch"${conf.theme === 'stopwatch' ? ' selected' : ''}>🎯 Stopwatch Mastery</option>
            <option value="cyberpunk"${conf.theme === 'cyberpunk' ? ' selected' : ''}>✨ Simple Modern</option>
            <option value="neonquest"${conf.theme === 'neonquest' ? ' selected' : ''}>⚔️ Neon Quest (Gamified Dark)</option>
            <option value="luminous"${conf.theme === 'luminous' ? ' selected' : ''}>✨ Luminous Glass (Modern Pro)</option>
            <option value="slate"${conf.theme === 'slate' ? ' selected' : ''}>🌑 Minimalist Slate (Matte Dark)</option>
            <option value="obsidian"${conf.theme === 'obsidian' ? ' selected' : ''}>🌌 Obsidian Dark</option>
            <option value="sapphire"${conf.theme === 'sapphire' ? ' selected' : ''}>💎 Midnight Sapphire</option>
            <option value="emerald"${conf.theme === 'emerald' ? ' selected' : ''}>🌿 Emerald Forest</option>
            <option value="amber"${conf.theme === 'amber' ? ' selected' : ''}>🌅 Warm Sunset</option>
            <option value="purple"${conf.theme === 'purple' ? ' selected' : ''}>🔮 Rose Velvet</option>
            <option value="light"${conf.theme === 'light' ? ' selected' : ''}>☀️ Clean Studio</option>
          </select>
        </div>
      </div>

      <!-- 1. METRIC CARDS GRID (PROGRESS, AVIRAL, TIME, TODAY) -->
      <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(170px, 1fr));gap:12px;margin-bottom:18px">
        <!-- PROGRESS -->
        <div style="background:#17181c;border:1px solid #23242a;border-radius:16px;padding:16px 14px;text-align:center;display:flex;flex-direction:column;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(0,0,0,0.25)">
          <span style="font-size:11px;font-weight:700;letter-spacing:0.08em;color:#8e909a;text-transform:uppercase;margin-bottom:2px">PROGRESS</span>
          <span id="easyStatProgressVal" style="font-size:26px;font-weight:300;color:#ffffff;margin:2px 0">${progressPercent}%</span>
          <span id="easyStatProgressSub" style="font-size:11px;color:#8e909a">${done}/${tot} days done</span>
        </div>

        <!-- AVIRAL (STREAK) -->
        <div style="background:#17181c;border:1px solid #23242a;border-radius:16px;padding:16px 14px;text-align:center;display:flex;flex-direction:column;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(0,0,0,0.25)">
          <span style="font-size:11px;font-weight:700;letter-spacing:0.08em;color:#8e909a;text-transform:uppercase;margin-bottom:2px">AVIRAL (STREAK)</span>
          <span id="easyStatStreakVal" style="font-size:26px;font-weight:300;color:#ffffff;margin:2px 0">${streak}</span>
          <span style="font-size:11px;color:#8e909a">days in a row</span>
        </div>

        <!-- TIME -->
        <div style="background:#17181c;border:1px solid #23242a;border-radius:16px;padding:16px 14px;text-align:center;display:flex;flex-direction:column;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(0,0,0,0.25)">
          <span style="font-size:11px;font-weight:700;letter-spacing:0.08em;color:#8e909a;text-transform:uppercase;margin-bottom:2px">TIME</span>
          <span id="easyStatTimeVal" style="font-size:26px;font-weight:300;color:#ffffff;margin:2px 0">${totalMinsText}</span>
          <span style="font-size:11px;color:#8e909a">total logged</span>
        </div>

        <!-- TODAY -->
        <div style="background:#17181c;border:1px solid #23242a;border-radius:16px;padding:16px 14px;text-align:center;display:flex;flex-direction:column;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(0,0,0,0.25)">
          <span style="font-size:11px;font-weight:700;letter-spacing:0.08em;color:#8e909a;text-transform:uppercase;margin-bottom:2px">TODAY</span>
          <span id="easyStatTodayVal" style="font-size:26px;font-weight:300;color:#ffffff;margin:2px 0">${todayHoursFormatted}</span>
          <span id="easyStatTodaySub" style="font-size:11px;color:#8e909a">of ${targetDailyHours}h target</span>
        </div>
      </div>
      ...
      <div style="background:#17181c;border:1px solid #23242a;border-radius:16px;padding:10px 10px;display:flex;align-items:center;justify-content:space-between;box-shadow:0 4px 20px rgba(0,0,0,0.25);margin-bottom:18px;max-width:100%;overflow:hidden">
        ...
        <div style="display:grid;grid-template-columns:repeat(5, 1fr);gap:8px;flex:1;margin:0 10px">
          ${daySelectorHtml}
        </div>

        <button onclick="jumpTo(${Math.min(totalDays - 1, curDay + 1)})" ${curDay >= totalDays - 1 ? 'disabled' : ''}
      ...
      <!-- 3. HOURS PLAN BAR CHART -->
      <div style="background:#17181c;border:1px solid #23242a;border-radius:16px;padding:18px 22px;box-shadow:0 4px 20px rgba(0,0,0,0.25);margin-bottom:18px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
          <div style="font-size:16px;font-weight:700;color:#ffffff">Hours Plan</div>
          <div id="easyHoursPlanHeaderMeta" style="font-size:12px;color:#94a3b8">In Progress: <strong style="color:#38bdf8">${todayHoursFormatted}</strong> / ${targetDailyHours}h · Planned: ${totalPlannedHours}h</div>
        </div>
        <div>
          ${hoursPlanBarsHtml || '<div style="font-size:13px;color:#64748b">No subjects set for today.</div>'}
        </div>
      </div>

      <!-- 4. DAY DETAIL SECTION & SUBJECT TIMERS -->
      <div style="margin-bottom:18px">
        <div style="font-size:15px;font-weight:700;color:#ffffff;margin-bottom:12px;padding:0 4px">
          Day ${dayNum} | ${dateStr} | Target: ${targetDailyHours} hours
        </div>

        <div>
          ${subjectTimerCardsHtml || '<div style="font-size:13px;color:#64748b">No subjects found for this day.</div>'}
        </div>
      </div>
...
window.activeCyberSubjectId = window.activeCyberSubjectId || null;

function setCyberActiveSubject(bid) {
  window.activeCyberSubjectId = (window.activeCyberSubjectId === bid ? null : bid);
  renderCyberpunkView();
}
...
function toggleCyberSubjectComplete(bid, dayId) {
  const p = gp(bid);
  p.done = !p.done;
  sp();
  renderCyberpunkView();
...
function saveCyberDayNotes(dayId, text) {
  ...
  sd();
  sp();
  const statusEl = document.getElementById('cyberNotesStatus');
  if (statusEl) {
    statusEl.textContent = '✓ Saved';
    statusEl.style.opacity = '1';
    setTimeout(() => {
      if (statusEl) statusEl.style.opacity = '0.6';
    }, 1500);
  }
}

function appendCyberNoteTemplate(dayId, templateText) {
  const ta = document.getElementById('cyberDayNotesArea');
  if (ta) {
    const curVal = ta.value ? ta.value + '\n' : '';
    ta.value = curVal + templateText;
    saveCyberDayNotes(dayId, ta.value);
  }
}

function renderCyberpunkTick() {
  checkDailyRollover();
  if (conf.theme !== 'cyberpunk') return;
  ...
  const totalSec = days.reduce((s, d) => s + d.blocks.reduce((ss, bk) => ss + (gp(bk.id).timeSpent || 0) + (timers[bk.id]?.running ? Math.floor((Date.now() - timers[bk.id].start) / 1000) : 0), 0), 0);
  const th = Math.floor(totalSec / 3600), tm = Math.floor((totalSec % 3600) / 60);
  const totalTimeText = totalSec < 3600 ? `${tm} mins` : `${(totalSec / 3600).toFixed(1)} hrs`;
  ...
  const progValEl = document.getElementById('cyberStatProgressVal');
  if (progValEl) progValEl.textContent = `${tot ? Math.round(done / tot * 100) : 0}%`;
  const progSubEl = document.getElementById('cyberStatProgressSub');
  if (progSubEl) progSubEl.textContent = `${done}/${tot} days done`;

  const streakValEl = document.getElementById('cyberStatStreakVal');
  if (streakValEl) streakValEl.textContent = `${streak}`;

  const timeValEl = document.getElementById('cyberStatTimeVal');
  if (timeValEl) timeValEl.textContent = totalTimeText;

  const todayValEl = document.getElementById('cyberStatTodayVal');
  if (todayValEl) todayValEl.textContent = `${todayLog}h`;
  const todaySubEl = document.getElementById('cyberStatTodaySub');
  if (todaySubEl) todaySubEl.textContent = `of ${todayTgt}h target`;

  // Update running timer button
  (curDayObj?.blocks || []).forEach(b => {
    const isRunning = !!timers[b.id]?.running;
    const btnEl = document.getElementById('cyberBtn-' + b.id);
    if (btnEl) {
      if (isRunning) {
        btnEl.innerHTML = `<span>Pause</span><span style="font-size:11px">⏸</span>`;
        btnEl.style.background = '#fbbf24';
        btnEl.style.color = '#000';
      } else {
        btnEl.innerHTML = `<span>Start</span><span style="font-size:11px">▶</span>`;
  ...
  if (runningBid) {
    ...
    const clockEl = document.getElementById('cyberClock-' + runningBid);
    if (clockEl) clockEl.textContent = clockStr;
    ...
    if (b) {
      ...
      const ringEl = document.getElementById('cyberPctRing-' + runningBid);
      if (ringEl) ringEl.textContent = `${pctVal}%`;

      const headerSubEl = document.getElementById('cyberHeaderSub-' + runningBid);
      if (headerSubEl) headerSubEl.textContent = `${pctVal}% · ${hoursDec}/${b.targetHrs} hours · ${b.topic || 'General'}`;

      const barFillEl = document.getElementById('cyberBarFill-' + runningBid);
      if (barFillEl) barFillEl.style.width = `${pctVal}%`;
      
      const targetLoggedEl = document.getElementById('cyberTargetLogged-' + runningBid);
      if (targetLoggedEl) targetLoggedEl.textContent = `${hoursDec}h / ${b.targetHrs}h`;
    }
  }
}

function renderCyberpunkView() {
  const container = document.getElementById('dayContent');
  if (!container) return;
  ...
  // 2. Day Selection Ribbon (18 Days centered around curDay)
  const ribbonCount = 19;
  const half = Math.floor(ribbonCount / 2);
  let startDay = Math.max(1, dayNum - half);
  let endDay = startDay + ribbonCount - 1;
  if (endDay > totalDays) {
    endDay = totalDays;
    startDay = Math.max(1, endDay - ribbonCount + 1);
  }

  let ribbonDaysHtml = '';
  for (let d = startDay; d <= endDay; d++) {
    const isSelected = d === dayNum;
    const isCompleted = dPct(d - 1) >= 100;

    if (isSelected) {
      ribbonDaysHtml += `
        <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer" onclick="jumpTo(${d - 1})">
          <div style="width:34px;height:34px;border-radius:50%;background:#2563eb;color:#ffffff;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;box-shadow:0 0 16px rgba(37,99,235,0.6)">
            ${d}
          </div>
          ${isCompleted ? '<div style="width:16px;height:3px;background:#10b981;border-radius:2px;margin-top:3px"></div>' : '<div style="height:3px;margin-top:3px"></div>'}
        </div>`;
    } else {
      ribbonDaysHtml += `
        <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;padding:2px" onclick="jumpTo(${d - 1})">
          <div style="width:34px;height:34px;border-radius:50%;color:#94a3b8;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:600" onmouseover="this.style.color='#ffffff'" onmouseout="this.style.color='#94a3b8'">
            ${d}
          </div>
          ${isCompleted ? '<div style="width:16px;height:3px;background:#10b981;border-radius:2px;margin-top:3px"></div>' : '<div style="height:3px;margin-top:3px"></div>'}
        </div>`;
  ...
  // 3. Segmented Day Hours Plan Bar
  let totalPlannedHours = 0;
  const blocks = curDayObj?.blocks || [];
  blocks.forEach(b => {
    const s = sj(b.subjectId);
    totalPlannedHours += (b.targetHrs || s.defaultHrs || 3);
  });

  const segmentedBarSegmentsHtml = blocks.map((b, idx) => {
    const s = sj(b.subjectId);
    const p = gp(b.id);
    const plannedHrs = b.targetHrs || s.defaultHrs || 3;
    const bSec = (p.timeSpent || 0) + (timers[b.id]?.running ? Math.floor((Date.now() - timers[b.id].start) / 1000) : 0);
    const segPct = totalPlannedHours > 0 ? (plannedHrs / totalPlannedHours) * 100 : 33.3;
    const innerFillPct = Math.min(100, Math.round((bSec / (plannedHrs * 3600)) * 100));

    return `
      <div style="position:relative;width:${segPct}%;height:100%;border-radius:9999px;background-color:${s.color};overflow:hidden;cursor:pointer" onclick="setCyberActiveSubject('${b.id}')" title="${esc(s.name)}: ${plannedHrs}h">
        <div id="cyberBarFill-${b.id}" style="position:absolute;left:0;top:0;bottom:0;width:${innerFillPct}%;background:rgba(255,255,255,0.3);border-radius:9999px;transition:width 0.4s ease-out"></div>
      </div>`;
  }).join('');

  const segmentedLegendHtml = blocks.map((b, idx) => {
    const s = sj(b.subjectId);
    const plannedHrs = b.targetHrs || s.defaultHrs || 3;
    return `
      <div style="display:flex;align-items:center;gap:6px;font-size:12px;color:#e2e8f0;cursor:pointer" onclick="setCyberActiveSubject('${b.id}')">
        <span style="font-size:14px">${s.icon || '⏱️'}</span>
        <span style="font-weight:700">${esc(s.name)}:</span>
        <span style="color:#94a3b8">${plannedHrs}h</span>
      </div>`;
  }).join('');

  // 4. Subject Timer Cards (Accordion)
  const activeSubjectId = window.activeCyberSubjectId || blocks[0]?.id;

  const subjectCardsHtml = blocks.map(b => {
    const s = sj(b.subjectId);
    const p = gp(b.id);
    let sec = p.timeSpent || 0;
    const isRunning = !!timers[b.id]?.running;
    ...
    const clockStr = `${String(th).padStart(2, '0')} : ${String(tm).padStart(2, '0')} : ${String(ts).padStart(2, '0')}`;
    const targetSec = (b.targetHrs || 3) * 3600;
    const pctVal = Math.min(100, Math.round((sec / targetSec) * 100));
    const loggedHoursDec = (sec / 3600).toFixed(1);
    const isExpanded = b.id === activeSubjectId;

    if (isExpanded) {
      return `
        <div style="background:#121927;border:1px solid #1e293b;border-radius:18px;padding:20px 22px;margin-bottom:14px;box-shadow:0 4px 20px rgba(0,0,0,0.3)">
          <!-- Header Bar -->
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;cursor:pointer" onclick="setCyberActiveSubject('${b.id}')">
            <div>
              <div style="display:flex;align-items:center;gap:8px;font-size:16px;font-weight:800;color:#ffffff">
                <span style="color:${s.color};font-size:14px">●</span>
                <span>${esc(s.name)}</span>
              </div>
              <div id="cyberHeaderSub-${b.id}" style="font-size:12px;color:#94a3b8;margin-top:4px">
                ${pctVal}% · ${loggedHoursDec}/${b.targetHrs} hours · ${esc(b.topic || 'Topic 2')}
              </div>
            </div>
            <div style="width:28px;height:28px;border-radius:8px;background:#1e293b;display:flex;align-items:center;justify-content:center;color:#94a3b8;font-size:11px">
              ▲
            </div>
          </div>

          <!-- Hero Stopwatch Control Box -->
          <div style="background:#0b0f19;border:1px solid #1e293b;border-radius:14px;padding:20px 24px;display:flex;align-items:center;justify-content:space-between;gap:18px;margin-bottom:16px;flex-wrap:wrap">
            <!-- Left Large Digital Clock -->
            <div style="font-family:'Space Mono',monospace;font-size:36px;font-weight:800;letter-spacing:0.08em;color:#ffffff;min-width:210px" id="cyberClock-${b.id}">
              ${clockStr}
            </div>

            <!-- Middle Actions: Start Button + Manual Add Input -->
            <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
              <button id="cyberBtn-${b.id}" onclick="toggleTimer('${b.id}','${curDayObj.id}')"
                      style="background:${isRunning ? '#fbbf24' : '#2563eb'};color:${isRunning ? '#000000' : '#ffffff'};border:none;border-radius:10px;padding:10px 24px;font-weight:800;font-size:14px;cursor:pointer;display:inline-flex;align-items:center;gap:8px;box-shadow:0 4px 14px ${isRunning ? 'rgba(251,191,36,0.4)' : 'rgba(37,99,235,0.4)'};transition:all 0.15s ease">
                <span>${isRunning ? 'Pause' : 'Start'}</span>
                <span style="font-size:11px">${isRunning ? '⏸' : '▶'}</span>
              </button>

              <div style="display:flex;align-items:center;gap:6px;background:#121927;border:1px solid #1e293b;border-radius:10px;padding:6px 10px">
                <input id="cyberManual-${b.id}" type="number" value="30" style="width:36px;background:transparent;border:none;color:#ffffff;font-weight:700;font-size:13px;text-align:center;outline:none" />
                <span style="font-size:11px;color:#94a3b8">min</span>
                <button onclick="addManualMinutes('${b.id}','${curDayObj.id}')" style="background:#1e293b;border:none;color:#cbd5e1;font-size:11px;font-weight:700;padding:3px 8px;border-radius:6px;cursor:pointer">
                  + Add
                </button>
              </div>
            </div>

            <!-- Right Circular Percentage Badge -->
            <div id="cyberPctRing-${b.id}" style="width:48px;height:48px;border-radius:50%;border:3px solid ${s.color};display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;color:${s.color};background:rgba(0,0,0,0.3);flex-shrink:0">
              ${pctVal}%
            </div>
          </div>

          <!-- Bottom Meta Row: Quick Add Chips + Topic Tag + Notes Tag -->
          <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap">
            <div style="display:flex;align-items:center;gap:8px">
              <span style="font-size:11px;color:#64748b;font-weight:600">Quick add:</span>
              <button onclick="quickAddCyberMins('${b.id}','${curDayObj.id}',15)" style="background:#0b0f19;border:1px solid #1e293b;color:#cbd5e1;font-size:11px;font-weight:700;padding:4px 10px;border-radius:8px;cursor:pointer">+15m</button>
              <button onclick="quickAddCyberMins('${b.id}','${curDayObj.id}',30)" style="background:#0b0f19;border:1px solid #1e293b;color:#cbd5e1;font-size:11px;font-weight:700;padding:4px 10px;border-radius:8px;cursor:pointer">+30m</button>
              <button onclick="quickAddCyberMins('${b.id}','${curDayObj.id}',45)" style="background:#0b0f19;border:1px solid #1e293b;color:#cbd5e1;font-size:11px;font-weight:700;padding:4px 10px;border-radius:8px;cursor:pointer">+45m</button>
              <button onclick="quickAddCyberMins('${b.id}','${curDayObj.id}',60)" style="background:#0b0f19;border:1px solid #1e293b;color:#cbd5e1;font-size:11px;font-weight:700;padding:4px 10px;border-radius:8px;cursor:pointer">+60m</button>
            </div>

            <div style="display:flex;align-items:center;gap:10px">
              <div onclick="editTopicModal('${curDayObj.id}','${b.id}')" style="display:flex;align-items:center;gap:6px;font-size:11px;color:#94a3b8;background:#0b0f19;border:1px solid #1e293b;padding:4px 10px;border-radius:8px;cursor:pointer" title="Click to edit topic">
                <span>🏷️</span>
                <span>Topic: <strong style="color:#ffffff">${esc(b.topic || 'Topic 2')}</strong></span>
              </div>
              <div onclick="openDayNotesModal('${curDayObj.id}')" style="display:flex;align-items:center;gap:6px;font-size:11px;color:#94a3b8;background:#0b0f19;border:1px solid #1e293b;padding:4px 10px;border-radius:8px;cursor:pointer" title="Open notes">
                <span>📄</span>
                <span>Notes •</span>
              </div>
            </div>
      ...
      return `
        <div style="background:#121927;border:1px solid #1e293b;border-radius:14px;padding:14px 20px;margin-bottom:10px;box-shadow:0 4px 16px rgba(0,0,0,0.25);display:flex;align-items:center;justify-content:space-between;cursor:pointer;transition:all 0.15s ease" onclick="setCyberActiveSubject('${b.id}')">
          <div>
            <div style="display:flex;align-items:center;gap:8px;font-size:15px;font-weight:800;color:#ffffff">
              <span style="color:${s.color};font-size:14px">●</span>
              <span>${esc(s.name)}</span>
            </div>
            <div style="font-size:12px;color:#94a3b8;margin-top:2px">
              ${pctVal}% · ${loggedHoursDec}/${b.targetHrs} hours · ${esc(b.topic || 'Topic 1')}
            </div>
          </div>
          <div style="width:28px;height:28px;border-radius:8px;background:#1e293b;display:flex;align-items:center;justify-content:center;color:#94a3b8;font-size:11px">
            ▼
  ...
  // 5. Bottom 2-Column Section (Subject Targets & Today's Sessions)
  const completedTargetsCount = blocks.filter(b => gp(b.id).done || (gp(b.id).timeSpent >= (b.targetHrs || 3) * 3600)).length;

  const subjectTargetsListHtml = blocks.map(b => {
    const s = sj(b.subjectId);
    const p = gp(b.id);
    const isDone = p.done || ((p.timeSpent || 0) >= (b.targetHrs || 3) * 3600);
    const loggedDec = ((p.timeSpent || 0) / 3600).toFixed(1);

    return `
      <div style="display:flex;align-items:center;justify-content:space-between;background:#0b0f19;border:1px solid #1e293b;border-radius:12px;padding:12px 14px;margin-bottom:8px">
        <div style="display:flex;align-items:center;gap:10px">
          <input type="checkbox" ${isDone ? 'checked' : ''} onchange="toggleCyberSubjectComplete('${b.id}','${curDayObj.id}')" style="width:16px;height:16px;accent-color:#2563eb;cursor:pointer" />
          <div>
            <div style="font-size:13px;font-weight:700;color:#ffffff">${esc(s.name)}</div>
            <div style="font-size:11px;color:#64748b">${esc(b.topic || 'Topic')} · ${b.targetHrs || 3}h planned</div>
          </div>
        </div>
        <div id="cyberTargetLogged-${b.id}" style="font-size:12px;font-weight:800;color:#94a3b8">${loggedDec}h / ${b.targetHrs || 3}h</div>
      </div>`;
  }).join('');

  // 6. Render Full Container
  container.innerHTML = `
    <div style="max-width:900px;margin:0 auto;padding:12px 6px">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(180px, 1fr));gap:14px;margin-bottom:18px">
        <!-- PROGRESS -->
        <div style="background:#121927;border:1px solid rgba(56,189,248,0.3);border-radius:18px;padding:16px 18px;box-shadow:0 4px 20px rgba(0,0,0,0.25)">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
            <span style="font-size:11px;font-weight:700;color:#94a3b8;letter-spacing:0.06em">PROGRESS</span>
            <span style="font-size:16px">🚩</span>
          </div>
          <div id="cyberStatProgressVal" style="font-size:28px;font-weight:800;color:#38bdf8;line-height:1.1">${progressPercent}%</div>
          <div id="cyberStatProgressSub" style="font-size:11px;color:#94a3b8;margin-top:4px">${done}/${tot} days done</div>
        </div>

        <!-- STREAK -->
        <div style="background:#121927;border:1px solid rgba(249,115,22,0.3);border-radius:18px;padding:16px 18px;box-shadow:0 4px 20px rgba(0,0,0,0.25)">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
            <span style="font-size:11px;font-weight:700;color:#94a3b8;letter-spacing:0.06em">STREAK</span>
            <span style="font-size:16px">🔥</span>
          </div>
          <div id="cyberStatStreakVal" style="font-size:28px;font-weight:800;color:#f97316;line-height:1.1">${streak}</div>
          <div style="font-size:11px;color:#94a3b8;margin-top:4px">days in a row</div>
        </div>

        <!-- TOTAL TIME -->
        <div style="background:#121927;border:1px solid rgba(52,211,153,0.3);border-radius:18px;padding:16px 18px;box-shadow:0 4px 20px rgba(0,0,0,0.25)">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
            <span style="font-size:11px;font-weight:700;color:#94a3b8;letter-spacing:0.06em">TOTAL TIME</span>
            <span style="font-size:16px">⏱️</span>
          </div>
          <div id="cyberStatTimeVal" style="font-size:28px;font-weight:800;color:#34d399;line-height:1.1">${totalTimeText}</div>
          <div style="font-size:11px;color:#94a3b8;margin-top:4px">total logged</div>
        </div>

        <!-- TODAY'S GOAL -->
        <div style="background:#121927;border:1px solid rgba(192,132,252,0.3);border-radius:18px;padding:16px 18px;box-shadow:0 4px 20px rgba(0,0,0,0.25)">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
            <span style="font-size:11px;font-weight:700;color:#94a3b8;letter-spacing:0.06em">TODAY'S GOAL</span>
            <span style="font-size:16px">📅</span>
          </div>
          <div id="cyberStatTodayVal" style="font-size:28px;font-weight:800;color:#c084fc;line-height:1.1">${todayHoursFormatted}</div>
          <div id="cyberStatTodaySub" style="font-size:11px;color:#94a3b8;margin-top:4px">of ${targetDailyHours}h target</div>
        </div>
      </div>

      <!-- 2. DAY SELECTION RIBBON -->
      <div style="background:#121927;border:1px solid #1e293b;border-radius:18px;padding:14px 18px;box-shadow:0 4px 20px rgba(0,0,0,0.3);margin-bottom:18px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
          <div style="font-size:14px;font-weight:800;color:#ffffff">Day Selection</div>
          <div style="font-size:11px;color:#94a3b8;font-family:'Space Mono',monospace">Day ${dayNum} of ${totalDays}</div>
        </div>

        <div style="display:flex;align-items:center;justify-content:space-between;gap:6px;overflow-x:auto">
          <button onclick="jumpTo(${Math.max(0, curDay - 1)})" ${curDay <= 0 ? 'disabled' : ''} style="background:transparent;border:none;color:#64748b;font-size:14px;cursor:pointer;padding:4px">◀</button>
          <div style="display:flex;align-items:center;gap:8px;flex:1;justify-content:center;overflow-x:auto;padding:2px 0">
            ${ribbonDaysHtml}
          </div>
          <button onclick="jumpTo(${Math.min(totalDays - 1, curDay + 1)})" ${curDay >= totalDays - 1 ? 'disabled' : ''} style="background:transparent;border:none;color:#64748b;font-size:14px;cursor:pointer;padding:4px">▶</button>
        </div>
      </div>
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;padding:0 4px">
        <div style="display:flex;align-items:baseline;gap:10px">
          <span style="font-size:26px;font-weight:900;color:#ffffff">Day ${dayNum}</span>
          <span style="font-size:13px;color:#94a3b8">${dateStr} · Target: ${targetDailyHours} hours</span>
        </div>
        <button onclick="openPlanModal()" style="background:#2563eb;color:#ffffff;border:none;border-radius:10px;padding:8px 18px;font-weight:800;font-size:13px;cursor:pointer;box-shadow:0 4px 14px rgba(37,99,235,0.4)">
          Manage Plan
        </button>
      </div>

      <!-- 5. SUBJECT TIMER CARDS (ACCORDION) -->
      <div style="margin-bottom:20px">
        ${subjectCardsHtml}
      </div>

      <!-- 6. BOTTOM 2-COLUMN SECTION -->
      <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(280px, 1fr));gap:16px">
        <!-- Left: Subject Targets -->
        <div style="background:#121927;border:1px solid #1e293b;border-radius:18px;padding:18px 20px;box-shadow:0 4px 20px rgba(0,0,0,0.3)">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
            <div style="display:flex;align-items:center;gap:6px;font-size:14px;font-weight:800;color:#38bdf8">
              <span>☑</span>
              <span>Day ${dayNum} Subject Targets</span>
            </div>
            <div style="font-size:11px;color:#64748b">${completedTargetsCount}/${blocks.length} completed</div>
          </div>
          <div>
            ${subjectTargetsListHtml}
          </div>
        </div>

        <!-- Right: Daily Notepad & Reflection -->
        <div style="background:#121927;border:1px solid #1e293b;border-radius:18px;padding:18px 20px;box-shadow:0 4px 20px rgba(0,0,0,0.3);display:flex;flex-direction:column;justify-content:space-between">
          <div>
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
              <div style="display:flex;align-items:center;gap:6px;font-size:14px;font-weight:800;color:#f97316">
                <span>📝</span>
                <span>Day ${dayNum} Notepad &amp; Reflection</span>
              </div>
              <div id="cyberNotesStatus" style="font-size:11px;font-weight:700;color:#10b981;opacity:0.8;transition:opacity 0.3s">
                ✓ Auto-saved
              </div>
            </div>

            <!-- Notepad Textarea -->
            <textarea id="cyberDayNotesArea"
              oninput="saveCyberDayNotes('${curDayObj.id}', this.value)"
              placeholder="Write today's key formulas, concepts memorized, chapter takeaways, or doubts for tomorrow..."
              style="width:100%;min-height:120px;background:#0b0f19;border:1px solid #1e293b;border-radius:12px;padding:12px 14px;color:#f8fafc;font-size:13px;line-height:1.5;resize:vertical;outline:none;font-family:inherit;box-shadow:inset 0 2px 6px rgba(0,0,0,0.3)">${esc(curDayObj.notes || gp(curDayObj.id).notes || '')}</textarea>
          </div>

          <!-- Quick Template Chips & Save Button -->
          <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:12px;flex-wrap:wrap">
            <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
              <button onclick="appendCyberNoteTemplate('${curDayObj.id}', '📌 Key Formulas:')" style="background:#0b0f19;border:1px solid #1e293b;color:#cbd5e1;font-size:11px;font-weight:700;padding:3px 8px;border-radius:6px;cursor:pointer">+ Formula</button>
              <button onclick="appendCyberNoteTemplate('${curDayObj.id}', '🎯 Tomorrow\\'s Focus:')" style="background:#0b0f19;border:1px solid #1e293b;color:#cbd5e1;font-size:11px;font-weight:700;padding:3px 8px;border-radius:6px;cursor:pointer">+ Goal</button>
              <button onclick="appendCyberNoteTemplate('${curDayObj.id}', '⚠️ Doubts / Blockers:')" style="background:#0b0f19;border:1px solid #1e293b;color:#cbd5e1;font-size:11px;font-weight:700;padding:3px 8px;border-radius:6px;cursor:pointer">+ Doubts</button>
            </div>
            <button onclick="saveCyberDayNotes('${curDayObj.id}', document.getElementById('cyberDayNotesArea')?.value || '')" style="background:#2563eb;color:#ffffff;border:none;border-radius:6px;padding:4px 10px;font-size:11px;font-weight:800;cursor:pointer;box-shadow:0 2px 8px rgba(37,99,235,0.4)">
              Save Note
            </button>
          </div>
        </div>
...
function resetTimerBlock(blockId, dayId) {
  if (timers[blockId]) {
    timers[blockId].running = false;
...
function editTopicModal(dayId, blockId) {
  const d = days.find(x => x.id === dayId);
  if (!d) return;
  const b = d.blocks.find(x => x.id === blockId);
  if (!b) return;
  const newTopic = prompt("Enter topic for " + sj(b.subjectId).name + ":", b.topic || "");
  if (newTopic !== null) {
    b.topic = newTopic.trim();
    sd();
    renderAll();
  }
}
...
function renderDaily() {
  ...
  if (conf.theme === 'cyberpunk') {
    document.body.setAttribute('data-tracker-mode', 'cyberpunk');
    if (statsRow) statsRow.style.display = 'none';
    if (hoursBar) hoursBar.style.display = 'none';
    ...
    renderCyberpunkView();
  } else if (conf.trackerMode === 'easy' || conf.theme === 'stopwatch') {
    document.body.setAttribute('data-tracker-mode', 'easy');
    if (statsRow) statsRow.style.display = 'none';
    if (hoursBar) hoursBar.style.display = 'none';
    if (todayBanner) todayBanner.style.display = 'none';
...
function renderDots() {
  ...
  if (conf.theme === 'neonquest') {
    let nodesHtml = days.map((_, i) => {
      ...
      if (isActive) {
        return `
          <div onclick="jumpTo(${i})" class="neon-level-node active" style="position:relative;display:flex;flex-direction:column;align-items:center;cursor:pointer;flex-shrink:0;margin:0 12px">
            <div style="width:46px;height:46px;border-radius:50%;border:2px solid #38bdf8;background:rgba(56,189,248,0.2);box-shadow:0 0 20px rgba(56,189,248,0.85), inset 0 0 10px rgba(56,189,248,0.4);display:flex;align-items:center;justify-content:center;font-family:'Space Mono',monospace;font-size:15px;font-weight:800;color:#38bdf8;transition:transform 0.2s">
              ${i + 1}
            </div>
            <span style="font-family:'Chakra Petch',sans-serif;font-size:9px;font-weight:800;color:#38bdf8;letter-spacing:0.08em;margin-top:6px;text-shadow:0 0 6px rgba(56,189,248,0.8)">CURRENT</span>
          </div>
        `;
      } else if (isDone) {
        return `
          <div onclick="jumpTo(${i})" class="neon-level-node done" style="position:relative;display:flex;flex-direction:column;align-items:center;cursor:pointer;flex-shrink:0;margin:0 12px">
            <div style="width:38px;height:38px;border-radius:50%;border:1.5px solid #22c55e;background:rgba(34,197,94,0.15);box-shadow:0 0 12px rgba(34,197,94,0.6);display:flex;align-items:center;justify-content:center;font-family:'Space Mono',monospace;font-size:13px;font-weight:700;color:#4ade80;transition:transform 0.2s">
              ${i + 1}
              <div style="position:absolute;top:-4px;right:-4px;width:16px;height:16px;border-radius:50%;background:#22c55e;border:1.5px solid #070c18;display:flex;align-items:center;justify-content:center;font-size:9px;color:#000;font-weight:900">✓</div>
            </div>
            <span style="font-family:'Chakra Petch',sans-serif;font-size:9px;font-weight:700;color:#4ade80;letter-spacing:0.08em;margin-top:6px">DONE</span>
          </div>
        `;
      } else {
        return `
          <div onclick="jumpTo(${i})" class="neon-level-node" style="position:relative;display:flex;flex-direction:column;align-items:center;cursor:pointer;flex-shrink:0;margin:0 12px">
            <div style="width:36px;height:36px;border-radius:50%;border:1.5px solid #1e2d4a;background:#0e1628;display:flex;align-items:center;justify-content:center;font-family:'Space Mono',monospace;font-size:12px;font-weight:600;color:#64748b;transition:border-color 0.2s">
              ${i + 1}
            </div>
            <span style="font-family:'Chakra Petch',sans-serif;font-size:9px;color:#475569;margin-top:6px">LVL ${i + 1}</span>
          </div>
        `;
      }
    }).join('');
    ...
    dotsEl.innerHTML = `
      <div style="width:100%;background:#091326;border:1.5px solid #1e2d4a;border-radius:14px;padding:14px 18px;box-shadow:0 0 16px rgba(0,0,0,0.4)">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
          <div style="font-family:'Orbitron',sans-serif;font-size:12px;font-weight:800;letter-spacing:0.08em;color:#94a3b8">
            LEVEL MAP <span style="color:#38bdf8;font-size:11px;margin-left:4px">(sector ${curDay + 1} active)</span>
          </div>
          <div style="font-family:'Chakra Petch',sans-serif;font-size:12px;font-weight:700;color:#38bdf8;cursor:pointer" onclick="switchView('manage')">All ${days.length} Sectors ›</div>
        </div>
        <div style="position:relative;display:flex;align-items:center;overflow-x:auto;padding:12px 6px;scrollbar-width:thin">
          <div style="position:absolute;left:20px;right:20px;top:32px;height:2px;background:#1e2d4a;z-index:0"></div>
    ...
    const activeNode = dotsEl.querySelector('.neon-level-node.active');
    if (activeNode) {
...
function toggleTrackerSound() {
  ...
  alert(window.soundEnabled ? "🔊 Sound FX enabled!" : "🔇 Sound FX muted.");
}

function renderNeonQuestDayContent(day) {
  const container = document.getElementById('dayContent');
  if (!container || !day) return;

  // Active hero block selection
  if (!window.activeNeonBlockId || !day.blocks.some(b => b.id === window.activeNeonBlockId)) {
    window.activeNeonBlockId = day.blocks[0]?.id;
  }
  const activeBlock = day.blocks.find(b => b.id === window.activeNeonBlockId) || day.blocks[0];
  const activeSubj = activeBlock ? sj(activeBlock.subjectId) : { name: 'Study', color: '#00b4d8', icon: '📝' };
  const activeProg = activeBlock ? gp(activeBlock.id) : { timeSpent: 0, subtopics: {} };

  // Timer state for active block
  let activeSec = activeProg.timeSpent || 0;
  const isRunning = !!(activeBlock && timers[activeBlock.id]?.running);
  if (isRunning) {
    activeSec += Math.floor((Date.now() - timers[activeBlock.id].start) / 1000);
  }
  const ah = Math.floor(activeSec / 3600), am = Math.floor((activeSec % 3600) / 60), as = activeSec % 60;
  const timerClockStr = `${String(ah).padStart(2, '0')} : ${String(am).padStart(2, '0')} : ${String(as).padStart(2, '0')}`;
  const targetHrs = activeBlock?.targetHrs || 3;
  const activePct = activeBlock ? bPct(activeBlock.id, activeBlock.subtopics) : 0;

  // Subject tabs for active quest
  const subjectTabsHtml = day.blocks.map(b => {
    const s = sj(b.subjectId);
    const isSel = b.id === activeBlock?.id;
    if (isSel) {
      return `<button onclick="window.activeNeonBlockId='${b.id}';renderDayContent();" style="padding:6px 14px;border-radius:8px;background:linear-gradient(180deg, rgba(14,165,233,0.2) 0%, rgba(3,105,161,0.4) 100%);border:1.5px solid #38bdf8;color:#38bdf8;font-family:'Chakra Petch',sans-serif;font-weight:800;font-size:12px;cursor:pointer;box-shadow:0 0 12px rgba(56,189,248,0.4);text-shadow:0 0 6px rgba(56,189,248,0.8)">${esc(s.name.toUpperCase())}</button>`;
    } else {
      return `<button onclick="window.activeNeonBlockId='${b.id}';renderDayContent();" style="padding:6px 14px;border-radius:8px;background:#0e1628;border:1px solid #1e2d4a;color:#94a3b8;font-family:'Chakra Petch',sans-serif;font-weight:700;font-size:12px;cursor:pointer;transition:all 0.2s" onmouseover="this.style.borderColor='#38bdf8';this.style.color='#f8fafc'" onmouseout="this.style.borderColor='#1e2d4a';this.style.color='#94a3b8'">${esc(s.name.toUpperCase())}</button>`;
    }
  }).join('');

  // Subtopics chips in active quest
  const subtopicsChipsHtml = (activeBlock?.subtopics || []).map((st, j) => {
    const isDone = !!activeProg.subtopics[j];
    return `<div onclick="toggleST('${activeBlock.id}', ${j}, '${day.id}')" style="padding:5px 12px;border-radius:8px;background:${isDone ? 'rgba(34,197,94,0.15)' : '#0e1628'};border:1px solid ${isDone ? '#22c55e' : '#1e2d4a'};color:${isDone ? '#4ade80' : '#cbd5e1'};font-size:11px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:6px" title="Toggle Done">${isDone ? '✓ ' : ''}${esc(st)}</div>`;
  }).join('');

  // 3-Column Subject Protocols
  const colorsPalette = ['#00b4d8', '#f97316', '#fbbf24', '#8b5cf6', '#22c55e'];
  const columnsHtml = day.blocks.map((b, idx) => {
    const s = sj(b.subjectId);
    const p = gp(b.id);
    const pct = bPct(b.id, b.subtopics);
    const bSec = (p.timeSpent || 0) + (timers[b.id]?.running ? Math.floor((Date.now() - timers[b.id].start) / 1000) : 0);
    const loggedH = (bSec / 3600).toFixed(1);
    const colTheme = colorsPalette[idx % colorsPalette.length];
    ...
    const topicsRows = (b.subtopics || []).map((st, j) => {
      ...
      `;
    }).join('') || '<div style="font-size:11px;color:#64748b;padding:8px 0">No subtopic modules.</div>';

    return `
      <div style="background:#091326;border:1.5px solid #1e2d4a;border-radius:14px;padding:16px;box-shadow:0 0 16px rgba(0,0,0,0.4);display:flex;flex-direction:column;justify-content:space-between">
        <div>
          <!-- Top Row: Badge, Name, % -->
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
            <span style="padding:3px 8px;font-size:10px;font-weight:800;border-radius:6px;background:#121c32;color:${colTheme};border:1px solid ${colTheme}60">${b.targetHrs}H QUEST</span>
            <div style="text-align:right">
              <div style="font-family:'Chakra Petch',sans-serif;font-size:14px;font-weight:800;color:#f8fafc">${pct}%</div>
              <div style="font-size:10px;color:#94a3b8">${loggedH}h / ${b.targetHrs}h</div>
            </div>
          </div>
          
          <div style="font-family:'Chakra Petch',sans-serif;font-size:16px;font-weight:800;color:#f8fafc;margin-bottom:10px">${esc(s.name)}</div>
          
          <!-- Progress Line -->
          <div style="width:100%;height:4px;background:#030712;border-radius:9999px;overflow:hidden;margin-bottom:14px">
            <div style="height:100%;width:${pct}%;background:${colTheme};box-shadow:0 0 8px ${colTheme}"></div>
          </div>

          <!-- Modules List -->
          <div style="font-family:'Chakra Petch',sans-serif;font-size:11px;font-weight:700;color:#64748b;letter-spacing:0.08em;margin-bottom:8px">📖 MODULES (${b.subtopics?.length || 0})</div>
          <div>${topicsRows}</div>
        </div>

        <div style="margin-top:12px;padding-top:10px;border-top:1px solid #1e2d4a;display:flex;justify-content:space-between;align-items:center">
          <button onclick="toggleTimer('${b.id}', '${day.id}')" style="flex:1;padding:6px 12px;border-radius:8px;background:${timers[b.id]?.running ? 'rgba(239,68,68,0.2)' : 'rgba(56,189,248,0.15)'};border:1.5px solid ${timers[b.id]?.running ? '#ef4444' : '#38bdf8'};color:${timers[b.id]?.running ? '#ef4444' : '#38bdf8'};font-weight:800;font-size:11px;cursor:pointer">
            ${timers[b.id]?.running ? '⏸ Pause Timer' : '▶ Start Timer'}
          </button>
        </div>
      </div>
    `;
  ...
  container.innerHTML = `
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin:18px 0 14px">
      <div>
        <div style="font-family:'Orbitron',sans-serif;font-size:24px;font-weight:900;letter-spacing:0.05em;color:#f8fafc">DAY ${curDay + 1}</div>
        <div style="font-size:13px;color:#94a3b8;margin-top:2px">${fd(getDd(curDay))} • Target: <strong style="color:#f8fafc">${day.targetHrs || 8} hours</strong></div>
      </div>
      <div style="display:flex;gap:8px;align-items:center">
        <button onclick="toggleTrackerSound()" style="padding:8px 12px;border-radius:8px;background:#0c1830;border:1px solid rgba(6,182,212,0.4);color:#38bdf8;font-size:14px;cursor:pointer" title="Toggle Sound">🔊</button>
        <button onclick="switchView('manage')" style="padding:8px 16px;border-radius:8px;background:#0c1830;border:1px solid rgba(6,182,212,0.6);color:#38bdf8;font-weight:700;font-size:12px;cursor:pointer;box-shadow:0 0 12px rgba(6,182,212,0.2)">Manage Plan ⚙️</button>
      </div>
    </div>

    <!-- HERO CURRENT QUEST CARD -->
    <div style="background:#091326;border:1.5px solid #00b4d8;border-radius:16px;padding:24px;box-shadow:0 0 24px rgba(0,180,216,0.25), inset 0 0 12px rgba(0,180,216,0.06);margin-bottom:24px">
      <!-- Header inside Quest -->
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px">
        <div>
          <div style="font-family:'Chakra Petch',sans-serif;font-size:11px;font-weight:800;letter-spacing:0.12em;color:#38bdf8;text-transform:uppercase">CURRENT QUEST</div>
          <div style="font-family:'Orbitron',sans-serif;font-size:22px;font-weight:900;letter-spacing:0.06em;color:#f8fafc;margin-top:2px">${esc(activeSubj.name.toUpperCase())}</div>
        </div>
        <div style="width:48px;height:48px;border-radius:50%;border:2px solid #38bdf8;box-shadow:0 0 12px rgba(56,189,248,0.6);display:flex;align-items:center;justify-content:center;font-family:'Chakra Petch',sans-serif;font-weight:800;font-size:13px;color:#38bdf8">
          ${activePct}%
        </div>
      </div>

      <!-- Giant Digital Timer Row -->
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;margin:16px 0 20px">
        <div class="timer-display" id="td-${activeBlock?.id}" style="font-family:'Space Mono',monospace;font-size: clamp(2rem, 5vw, 3.2rem);font-weight:700;letter-spacing:0.14em;color:#38bdf8;text-shadow:0 0 18px rgba(56,189,248,0.9), 0 0 35px rgba(56,189,248,0.45);margin-bottom:18px">
          ${timerClockStr}
        </div>

        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;justify-content:center">
          <button class="neon-button-blue" onclick="toggleTimer('${activeBlock?.id}', '${day.id}')" style="padding:10px 28px;border-radius:10px;font-family:'Chakra Petch',sans-serif;font-weight:800;font-size:14px;letter-spacing:0.08em;cursor:pointer">
            ${isRunning ? '⏸ PAUSE MISSION' : '▶ MISSION START'}
          </button>
          <div style="display:flex;align-items:center;gap:6px">
            <input type="number" id="mi-${activeBlock?.id}" placeholder="min" min="1" style="width:64px;padding:8px 10px;border-radius:8px;background:#0e1628;border:1.5px solid #1e2d4a;color:#f8fafc;font-size:12px;outline:none">
            <button onclick="addManTime('${activeBlock?.id}', '${day.id}')" style="padding:8px 14px;border-radius:8px;background:#0e1628;border:1.5px solid #38bdf8;color:#38bdf8;font-weight:700;font-size:12px;cursor:pointer">+Add</button>
          </div>
        </div>
      </div>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:8px;margin-bottom:20px;flex-wrap:wrap">
        <span style="font-size:11px;color:#64748b;font-weight:700;margin-right:4px">Quick Log:</span>
        <button onclick="quickAddNeonMinutes('${activeBlock?.id}', '${day.id}', 15)" style="padding:4px 10px;border-radius:6px;background:#0e1628;border:1px solid #1e2d4a;color:#94a3b8;font-size:11px;font-weight:700;cursor:pointer">+15m</button>
        <button onclick="quickAddNeonMinutes('${activeBlock?.id}', '${day.id}', 25)" style="padding:4px 10px;border-radius:6px;background:#0e1628;border:1px solid #1e2d4a;color:#94a3b8;font-size:11px;font-weight:700;cursor:pointer">+25m</button>
        <button onclick="quickAddNeonMinutes('${activeBlock?.id}', '${day.id}', 45)" style="padding:4px 10px;border-radius:6px;background:#0e1628;border:1px solid #1e2d4a;color:#94a3b8;font-size:11px;font-weight:700;cursor:pointer">+45m</button>
        <button onclick="quickAddNeonMinutes('${activeBlock?.id}', '${day.id}', 60)" style="padding:4px 10px;border-radius:6px;background:#0e1628;border:1px solid #1e2d4a;color:#94a3b8;font-size:11px;font-weight:700;cursor:pointer">+60m</button>
      </div>
    ...
    <!-- DAILY QUESTS & TOPIC PROTOCOLS SECTION -->
    <div style="margin-bottom:24px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
        <div style="font-family:'Orbitron',sans-serif;font-size:14px;font-weight:800;letter-spacing:0.08em;color:#f8fafc">DAILY QUESTS & TOPIC PROTOCOLS</div>
        <div style="font-size:12px;color:#94a3b8;font-weight:600">${day.blocks.length} Subjects Active</div>
      </div>
...
function toggleTimer(bid, dayId) {
  if (timers[bid]?.running) {
    ...
    timers[bid].interval = setInterval(() => {
      ...
      if (el) {
        el.textContent = conf.theme === 'neonquest'
      ...
      renderStats(); renderHoursBar(); renderEasyModeTick(); renderCyberpunkTick();
...
function renderDayRowHTML(d) {
  ...
  return `<div class="day-row-m" id="day-row-m-${d.id}" data-day-index="${d.i}">
...
function highlightDayInManage(dayIndexOrId) {
  let dayIdx = curDay;
  if (typeof dayIndexOrId === 'number') {
    dayIdx = dayIndexOrId;
  } else if (typeof dayIndexOrId === 'string') {
    const foundIdx = days.findIndex(d => d.id === dayIndexOrId);
    if (foundIdx !== -1) dayIdx = foundIdx;
  }

  if (dayIdx < 0 || dayIdx >= days.length) dayIdx = 0;

  const targetWeek = Math.floor(dayIdx / 7);

  if (window.collapsedWeeks) {
    window.collapsedWeeks.delete(targetWeek);
  }

  if (conf.activeTab !== 'manage') {
    conf.activeTab = 'manage';
    document.querySelectorAll('.tab').forEach(b => b.classList.toggle('active', b.dataset.view === 'manage'));
    document.querySelectorAll('.view').forEach(x => x.classList.toggle('active', x.id === 'view-manage'));
  }
  ...
  setTimeout(() => {
    ...
    const contentEl = document.getElementById(`week-cnt-${targetWeek}`);
    const headerEl = document.getElementById(`week-hdr-${targetWeek}`);
    if (contentEl && contentEl.classList.contains('collapsed')) {
      contentEl.classList.remove('collapsed');
      if (headerEl) {
        headerEl.classList.remove('collapsed');
        const arrow = headerEl.querySelector('.week-arrow');
        if (arrow) arrow.textContent = '▼';
      }
    }

    const rowEl = document.getElementById(`day-row-m-${targetDay.id}`);
    if (rowEl) {
      rowEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
      rowEl.classList.remove('highlight-blink');
      void rowEl.offsetWidth;
      rowEl.classList.add('highlight-blink');
      setTimeout(() => {
        rowEl.classList.remove('highlight-blink');
      }, 3000);
    }
...
function saveDay() {
  ...
  if (editDayId) {
    ...
    if (d) {
      d.title = title; d.dateOverride = dv ? new Date(dv).toISOString() : null; d.targetHrs = target;
...
function setAppTheme(themeKey) {
  ...
  // Sync Quick Theme Dropdown in Daily View
  ...
  try {
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type: 'TOGGLE_THEME', theme: themeKey }, '*');
      if (window.parent.document && window.parent.document.documentElement) {
        window.parent.document.documentElement.setAttribute('data-theme', themeKey);
        window.parent.document.documentElement.classList.toggle('dark', !isLight);
      }
    }
  } catch (e) {}
